...
const TIPO_PARCELA_MAP = {
  ...
  liberacao:      { label: 'Liberação',      cls: 'dot-lib',     cor: 'var(--ouro)' },
  entrega_parcial:{ label: 'Entrega Parcial',cls: 'dot-parcial', cor: 'var(--consul)' },
  conclusao:      { label: 'Conclusão',      cls: 'dot-conc',    cor: 'var(--verde)' },
...
function nucleoBadge(n) {
  ...
  if (!m) return `<span class="badge b-gr">${_finEsc(n||'—')}</span>`;
...
function sitBadge(s) {
  const m = SIT_LANC[s];
  if (!m) return `<span class="badge b-gr">${_finEsc(s||'—')}</span>`;
  return `<span class="badge ${m.cls}">${_finEsc(m.label)}</span>`;
}
...
async function _preencherLembrePara() {
  ...
  sel.innerHTML = '<option value="todos">Toda a equipe</option>' +
    (usuarios || []).filter(u => u.id !== _me.id)
      .map(u => `<option value="${u.id}">${(u.nome||'').split(' ')[0]}</option>`).join('');
}
function fecharToolFeedbackPopover() {
  document.getElementById('nav-feedback-pop').classList.remove('open');
}
...
// ══════════════════════════════════════════════════════════
//   NOTIFICAÇÕES
// (painel de notificações — gerenciado por shared/exp-nav.js)
// carregarNotificacoes mantido para compatibilidade futura
// ══════════════════════════════════════════════════════════
async function carregarNotificacoes() {
  const list = document.getElementById('notif-list');
  if (!list) return; /* painel local removido; exp-nav cuida das notificações */
  ...
  const [
    ...
    // 1. Rascunhos aguardando ativação (do CRM ou manual)
    sb.from('fin_ativos')
    ...
    // 2. Parcelas com NF a emitir (inclui entregue = etapa concluída sem NF)
    ...
    // 3. Cobranças VENCIDAS (aguardando pagamento e data já passou)
    sb.from('fin_parcelas')
    ...
    // 4. Cobranças vencendo nos próximos 7 dias
    sb.from('fin_parcelas')
      .select('id,ativo_id,descricao,valor,data_vencimento,fin_ativos!inner(cliente_nome,num_legado)')
      .eq('situacao', 'aguardando_pagamento')
    ...
    // 5. Despesas/lançamentos VENCIDOS (a_pagar com data passada)
  ...
  if (rascunhos?.length) {
    ...
    if (crmRasc.length) {
      ...
      itens.push(...crmRasc.map(r => ({
        ...
        icon: '🆕',
        texto: `${r.cliente_nome || 'Novo projeto'} · ${r.nucleo || ''}`,
        sub: `Rascunho criado pelo CRM · ${fmtData(r.created_at?.split('T')[0])}`,
      })));
    }
    if (manRasc.length) {
      itens.push(...manRasc.map(r => ({
        ...
        icon: '📁',
        texto: `${r.cliente_nome || 'Projeto sem nome'} · ${r.nucleo || ''}`,
        sub: `Rascunho aguardando ativação · ${fmtData(r.created_at?.split('T')[0])}`,
      })));
    }
    grupos.push({ label: 'Novos Projetos', itens });
  }

  // ── Grupo: NF a emitir (entregue = etapa concluída, a_emitir = aguardando emissão)
  if (nfEmitir?.length) {
    ...
    grupos.push({
      ...
      itens: nfEmitir.map(p => ({
        ...
        icon: p.situacao === 'entregue' ? '✅' : '🧾',
        texto: `${p.fin_ativos?.cliente_nome || '—'} · ${p.descricao || 'Parcela'}`,
        sub: p.situacao === 'entregue'
          ? 'Etapa concluída · aguardando emissão da NF'
          : `NF a emitir${p.data_prevista_cobranca ? ' · ' + fmtData(p.data_prevista_cobranca) : ''}`,
        val: p.valor ? moeda(p.valor, true) : null,
      })),
    });
  }

  // ── Grupo: Cobranças vencidas ─────────────────────────
    ...
    grupos.push({
      ...
      itens: cobVencidas.map(p => {
        ...
        return {
          ...
          icon: '🔴',
          texto: `${p.fin_ativos?.cliente_nome || '—'} · ${p.descricao || 'Parcela'}`,
          sub: `Venceu há ${dias} dia${dias !== 1 ? 's' : ''} · ${fmtData(p.data_vencimento)}`,
          val: p.valor ? moeda(p.valor, true) : null,
        };
      }),
    });
    ...
    grupos.push({
      ...
      itens: cobVencendo.map(p => {
        ...
        return {
          ...
          icon: '⏰',
          texto: `${p.fin_ativos?.cliente_nome || '—'} · ${p.descricao || 'Parcela'}`,
          sub: `Vence ${dias === 0 ? 'hoje' : 'em ' + dias + ' dia' + (dias !== 1 ? 's' : '')} · ${fmtData(p.data_vencimento)}`,
          val: p.valor ? moeda(p.valor, true) : null,
        };
      }),
    });
    ...
    grupos.push({
      ...
      itens: despVencidas.map(l => {
        ...
        return {
          ...
          icon: '💸',
          texto: l.observacao || 'Despesa sem descrição',
          sub: `Venceu há ${dias} dia${dias !== 1 ? 's' : ''} · ${fmtData(l.data_vencimento)}`,
  ...
  if (!list) return;
  if (!grupos.length) {
    list.innerHTML = '<div class="notif-empty">✓ Tudo em dia — nenhuma pendência</div>';
    return;
  }
...
document.addEventListener('click', e => {
  ...
  if (!e.target.closest('#lemb-wrap'))          document.getElementById('lemb-popdown')?.classList.remove('open');
});
function initConnDot() {
  const dots = [document.getElementById('conn-dot'), document.getElementById('conn-dot-dd')];
  const upd = () => {
    const cls = 'conn-dot ' + (navigator.onLine ? 'online' : 'offline');
    dots.forEach(d => { if (d) d.className = cls; });
    const lbl = document.getElementById('conn-label');
    if (lbl) lbl.textContent = navigator.onLine ? 'conectado' : 'sem conexão';
  };
...
function isSocioAdmFinanceiroRole(role) {
  return ['socio_adm', 'socio_admin'].includes((role || '').toLowerCase());
}
async function sair() {
...
function setAnaliseSubtema(subtema) {
  G_analise.subtema = subtema || 'resumo';
  document.querySelectorAll('[data-asub]').forEach((b) => {
    const ativo = b.getAttribute('data-asub') === G_analise.subtema;
    b.classList.toggle('btn-pri', ativo);
    b.classList.toggle('btn-sec', !ativo);
  });
...
async function carregarAnalise() {
  ...
  // Lançamentos do ano com plano de contas
...
function renderNucleoMargens() {
  ...
  const ano   = G_analise.ano;
  const subtema = G_analise.subtema || 'resumo';
  const MESES = ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'];
  ...
  dados.forEach(l => {
    ...
    // Custos diretos = despesas com nucleo_crm definido (não nulo = não é overhead)
  ...
  if (grid) {
    grid.innerHTML = NUCLEOS_ANALISE.map(n => {
      ...
      const temaValor = subtema === 'receita'
        ? moeda(d.rec)
        : subtema === 'custos'
          ? moeda(d.custo)
          : subtema === 'margem'
            ? moeda(margem)
            : moeda(d.rec);
      const temaCor = subtema === 'custos'
        ? 'var(--terracota)'
        : subtema === 'margem'
          ? barCor
          : n.cor;
      const temaLabel = subtema === 'receita'
        ? `Receita ${ano} · ${share.toFixed(1)}% do total`
        : subtema === 'custos'
          ? `Custos diretos ${ano}`
          : subtema === 'margem'
      ...
      <div class="analise-card" style="--ac:${n.cor}">
        ...
        <div class="analise-card-val" style="color:${temaCor}">${temaValor}</div>
        <div class="analise-card-lbl">${temaLabel}</div>
  ...
  // Cabeçalho
  tbl.querySelector('thead tr').innerHTML = '<th>Núcleo</th>' +
    MESES.map((m, i) => `<th style="text-align:right">${m}</th>`).join('') +
    '<th style="text-align:right">Total</th>';

  // Corpo: 1 linha por núcleo (receita), depois margem %
  ...
  NUCLEOS_ANALISE.forEach(n => {
    ...
    if (subtema === 'receita') {
      ...
      <tr>
        ...
        ${d.mesRec.map(v => `<td style="text-align:right">${v > 0 ? moeda(v) : '<span style="color:#ccc">—</span>'}</td>`).join('')}
    ...
    if (subtema === 'custos') {
      ...
      <tr>
        <td style="color:${n.cor};font-weight:700">${n.label}</td>
        ${d.mesCusto.map(v => `<td style="text-align:right">${v > 0 ? moeda(v) : '<span style="color:#ccc">—</span>'}</td>`).join('')}
        <td style="text-align:right;font-weight:700">${moeda(d.custo)}</td>
      </tr>`;
      return;
    }
    if (subtema === 'margem') {
      ...
      <tr>
        ...
        ${d.mesRec.map((rec, i) => {
          ...
          if (mg === null) return '<td style="text-align:right;color:#ccc;font-size:10px">—</td>';
    ...
    <tr style="background:var(--cinza2)">
      <td style="color:#999;font-size:10px;padding-left:20px">↳ Margem %</td>
      ${d.mesRec.map((rec, i) => {
        const mg = rec > 0 ? ((rec - d.mesCusto[i]) / rec * 100) : null;
        if (mg === null) return '<td style="text-align:right;color:#ccc;font-size:10px">—</td>';
        const c = mg >= 40 ? 'var(--verde)' : mg >= 20 ? 'var(--ouro)' : 'var(--terracota)';
  ...
  if (subtema === 'receita') {
    ...
    <tr class="an-total-row">
      <td>Total Receita</td>
      ${totalRec.mes.map(v => `<td style="text-align:right">${v > 0 ? moeda(v) : '—'}</td>`).join('')}
      <td style="text-align:right">${moeda(totalRec.val)}</td>
    </tr>`;
  } else if (subtema === 'custos') {
    const totalCustoMes = Array(12).fill(0);
    NUCLEOS_ANALISE.forEach(n => {
      agg[n.key].mesCusto.forEach((v, i) => totalCustoMes[i] += v);
    });
    const totalCusto = totalCustoMes.reduce((s, v) => s + v, 0);
    html += `
    <tr class="an-total-row">
      <td>Total Custos</td>
      ${totalCustoMes.map(v => `<td style="text-align:right">${v > 0 ? moeda(v) : '—'}</td>`).join('')}
      <td style="text-align:right">${moeda(totalCusto)}</td>
    </tr>`;
  } else {
    html += `
    <tr class="an-total-row">
      <td>${subtema === 'margem' ? 'Margem consolidada' : 'Total Geral'}</td>
      ${totalRec.mes.map(v => `<td style="text-align:right">${v > 0 ? moeda(v) : '—'}</td>`).join('')}
      <td style="text-align:right">${moeda(totalRec.val)}</td>
    </tr>`;
  }
...
function renderProjetoMargens() {
  ...
  const rows = Object.entries(byAtivo)
    .map(([aid, d]) => {
      ...
      const nucleo = d.nucleo || ativo?.nucleo_crm || '—';
      const margem = d.rec - d.custo;
  ...
  if (!body) return;
  const head = document.querySelector('#analise-proj-tbl thead tr');
  if (head) {
    head.innerHTML = subtema === 'receita'
      ? '<th style="width:260px">Projeto / Ativo</th><th class="r">Núcleo</th><th class="r">Receita</th><th class="r">% da receita</th>'
      : subtema === 'custos'
        ? '<th style="width:260px">Projeto / Ativo</th><th class="r">Núcleo</th><th class="r">Custos Diretos</th><th class="r">% sobre receita</th>'
        : subtema === 'margem'
          ? '<th style="width:260px">Projeto / Ativo</th><th class="r">Núcleo</th><th class="r">Margem Bruta</th><th class="r">% Margem</th>'
          : '<th style="width:260px">Projeto / Ativo</th><th class="r">Núcleo</th><th class="r">Receita</th><th class="r">Custos Diretos</th><th class="r">Margem Bruta</th><th class="r">% Margem</th>';
  }

  if (!rows.length) {
    body.innerHTML = `<tr><td colspan="${subtema === 'resumo' ? 6 : 4}" style="text-align:center;color:#999;padding:30px">Nenhum dado encontrado para ${G_analise.ano}</td></tr>`;
    return;
  }
  ...
  body.innerHTML = rows.map(r => {
    ...
    if (subtema === 'receita') {
      const share = maxRec > 0 ? (r.rec / maxRec) * 100 : 0;
      return `
      <tr>
        <td>
        ...
        <td class="r" style="color:${nucCor};font-weight:700">${share.toFixed(1)}%</td>
      </tr>`;
    }
    if (subtema === 'custos') {
      const pctCusto = r.rec > 0 ? (r.custo / r.rec * 100) : null;
      return `
      <tr>
        <td>
          <div style="font-weight:600;font-size:12px">${r.nome}</div>
        ...
        <td class="r" style="color:${pctCusto !== null && pctCusto > 80 ? 'var(--terracota)' : 'var(--ouro)'};font-weight:700">${pctCusto !== null ? pctCusto.toFixed(1) + '%' : '—'}</td>
      </tr>`;
    }
    if (subtema === 'margem') {
      return `
...
async function carregarPlanoContas() {
  ...
  const planOpt = G.planoContas.map(pc =>
    `<option value="${_finAttr(pc.id)}">${_finEsc(pc.grupo)} · ${_finEsc(pc.subconta)}</option>`
  ).join('');
  ['lanc-plano','rec-plano'].forEach(sid => {
    const el = document.getElementById(sid);
    if (el) el.innerHTML = '<option value="">Selecione…</option>' + planOpt;
  });
  // grupo filter
...
async function carregarDashboard() {
  ...
  // YTD: Jan até o mês selecionado (inclusive)
  const ytdMeses = [];
  ...
  const [
    ...
    // Receita mês atual (pago + aprovado)
    ...
    sb.from('fin_lancamentos').select('valor')
      ...
      .in('situacao',['pago','aprovado']),
    // Receita por nucleo (mês atual — join já feito acima, reutiliza recMes)
  ...
  const deltaBadge = (atual, anterior, pct = false, inverter = false) => {
    if (!anterior) return '<span class="kpi-delta-neu">— sem dado anterior</span>';
  ...
  if (margem !== null) {
    ...
    document.getElementById('kpi-margem-delta').innerHTML =
      margemP !== null ? deltaBadge(margem, margemP, true) : '<span class="kpi-delta-neu">— sem dado anterior</span>';
  } else {
  ...
  document.getElementById('kpi-acumulado-sub').textContent =
    `Jan → ${MESES_ABR[mes-1]} · ${mes} ${mes === 1 ? 'mês' : 'meses'}`;
  ...
  document.getElementById('kpi-projecao-sub').textContent =
    `média mensal × 12 · base: ${mes} ${mes === 1 ? 'mês' : 'meses'}`;
  ...
  document.getElementById('dash-nucleos').innerHTML = NC_DEF.map(n => {
    ...
    <div class="dash-nc-card" style="--ac:${n.cor}">
      ...
      <div class="dash-nc-sub">${pct}% da receita do mês</div>
...
// ── Instâncias dos charts (destruir antes de recriar) ────────
let _chartQuad = null;
let _chartNucleo = null;

async function renderDashCharts(mes, ano, totalRecMes, byNucleo, NC_DEF) {
  // Quadrimestre: últimos 4 meses até o selecionado (inclusive)
  const quadMeses = [];
  const quadLabels = [];
  for (let i = 3; i >= 0; i--) {
    let m = mes - i;
  ...
  // ── Chart 1: Quadrimestre (barras agrupadas + linha resultado) ──
  ...
  // ── Chart 2: Receita por núcleo (donut) ──
...
async function carregarAlertas() {
  ...
  const [{ data: nfPend }, { data: cobVenc }] = await Promise.all([
    // NF pendente: etapas concluídas (entregue) e marcadas para emitir (a_emitir)
    sb.from('fin_parcelas')
  ...
  (cobVenc || []).forEach(p => {
    const dias = Math.floor((new Date() - new Date(p.data_vencimento + 'T12:00:00')) / 86400000);
    itens.push({ urgente: true, icon: '🔴', cor: 'var(--terracota)',
      cliente: p.fin_ativos?.cliente_nome, num: p.fin_ativos?.num_legado,
      desc: `${TIPO_PARCELA_MAP[p.tipo]?.label || p.tipo} · Venceu há ${dias}d`,
      valor: p.valor, id: p.id, acao: 'pgto',
    });
  });

  (nfPend || []).forEach(p => {
    itens.push({ urgente: true, icon: p.situacao === 'entregue' ? '✅' : '🧾', cor: 'var(--ouro)',
      cliente: p.fin_ativos?.cliente_nome, num: p.fin_ativos?.num_legado,
      desc: p.situacao === 'entregue' ? 'Etapa concluída · emitir NF' : 'NF a emitir',
      valor: p.valor, id: p.id, acao: 'nf',
    });
  });

  if (!itens.length) {
    el.innerHTML = '<div style="padding:20px;text-align:center;color:#bbb;font-size:11px">Sem pendências urgentes ✓</div>';
    return;
  }
    ...
    <div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--cinza2)">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-size:12px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
          ${_finEsc(i.cliente || '—')} <span style="font-size:10px;color:#888;font-weight:400">${_finEsc(i.num || '')}</span>
        </div>
...
async function carregarEtapasRecentes() {
  ...
  if (!data || data.length === 0) {
    el.innerHTML = '<div style="padding:20px;text-align:center;color:#bbb;font-size:11px">Nenhuma etapa recém-concluída</div>';
    return;
  }
  el.innerHTML = data.map(e => `
    <div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--cinza2)">
      <div style="flex:1">
        <div style="font-size:12px;font-weight:600">${_finEsc(e.fin_ativos?.cliente_nome||'—')}</div>
        <div style="font-size:10px;color:#888">${_finEsc(e.descricao || '')} · concluída ${fmtData(e.data_entrega_realizada)}</div>
      </div>
...
async function carregarFila() {
  ...
  el.innerHTML = '<div style="padding:20px;text-align:center;color:#bbb;font-size:11px">Carregando…</div>';
  ...
  el.innerHTML = data.map(a => {
    ...
    return `<div class="ativo-card" onclick="abrirAtivo(${ativoIdJs})">
      <div class="ativo-card-hd">
        ...
        ${viaCRM ? `<span class="badge" style="background:#EEF2FF;color:#4338CA;border:1px solid #C7D2FE;font-size:8px">↑ CRM</span>` : ''}
        <button class="btn btn-pri btn-sm" onclick="event.stopPropagation();finalizarRascunho(${ativoIdJs})" style="margin-left:auto">Finalizar →</button>
      </div>
      ${a.descricao ? `<div style="font-size:11px;color:#666;padding:4px 0 2px">${descDisp}</div>` : ''}
      <div class="ativo-card-meta">
        ${a.num_legado ? `<span>📁 ${legadoDisp}</span>` : ''}
        ${a.prop_code  ? `<span>🧮 ${propCodeDisp}</span>`  : ''}
        ${a.data_contrato ? `<span>📅 ${fmtData(a.data_contrato)}</span>` : ''}
        ${viaCRM ? `<span title="Criado automaticamente pelo fechamento no CRM">⚡ Fechado no CRM</span>` : ''}
      </div>
...
async function carregarConcluidos() {
  ...
  if (!data?.length) { el.innerHTML = '<div class="empty-state"><div class="empty-state-title">Nenhum concluído</div></div>'; return; }
...
function ativoCardHTML(a, sit) {
  ...
  const sitLabel = { ativo:'Ativo', concluido:'Concluído', rascunho:'Rascunho', cancelado:'Cancelado' };
  const ativoIdJs = _finSq(a.id);
  const clienteDisp = _finEsc(a.cliente_nome || '');
  const legadoDisp = _finEsc(a.num_legado || '');
  const propCodeDisp = _finEsc(a.prop_code || '');
  return `<div class="ativo-card" onclick="abrirAtivo(${ativoIdJs})">
    ...
    <div class="ativo-card-meta">
      ${a.num_legado ? `<span>📁 ${legadoDisp}</span>` : ''}
      ${a.prop_code  ? `<span>🧮 ${propCodeDisp}</span>`  : ''}
      <span>📅 ${fmtData(a.data_contrato)}</span>
    </div>
    <div class="ativo-card-footer">
...
function abrirAtivo(id) { toast('Detalhes do ativo em breve…'); }
function abrirModalNovoAtivo() { toast('Cadastro manual em breve…'); }

// ══════════════════════════════════════════════════════════
//   COBRANÇA — SUB-TABS + AGING
// ══════════════════════════════════════════════════════════
function setCobTab(tab) {
  document.querySelectorAll('[data-cobtab]').forEach(b => {
    ...
    if (b.dataset.cobtab === 'clientes') b.style.borderBottomColor = tab === 'clientes' ? 'var(--fin)' : '';
  });
  ...
  document.getElementById('cob-tab-clientes').style.display = tab === 'clientes' ? '' : 'none';
  document.getElementById('cob-filtros-pipeline').style.display = tab === 'pipeline' ? '' : 'none';
  if (tab === 'aging') renderAging();
  if (tab === 'clientes') renderPainelClientesFinanceiros();
}
...
function renderAging() {
  ...
  const vencer  = com.filter(p => p.diff <= 0);   // não vencido (diff negativo = futuro)
  ...
  buckets.forEach(({ key, arr }) => {
    const v = soma(arr);
    document.getElementById(`ag-val-${key}`).textContent = v > 0 ? moeda(v) : '—';
  ...
  if (!vencidas.length) {
    el.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-state-sub">Todos os recebíveis estão dentro do prazo</div>
      </div>`;
    return;
  }
  ...
  el.innerHTML = vencidas.map(p => {
    ...
    const clienteDisp = _finEsc(p.fin_ativos?.cliente_nome || '—');
    ...
    <div class="aging-row">
      ...
      <div class="aging-row-info">
        ...
        <div class="aging-row-detalhe">
          ${legadoDisp} · ${tipoDisp}
          ${p.descricao ? ' · ' + descDisp : ''}
          · Venceu ${fmtData(p.data_vencimento)}
        </div>
      </div>
...
function _clienteFinDaysBetween(a, b) {
  if (!a || !b) return null;
...
function _clienteFinEntidadeData(parcela) {
  const payload = finExtrairPayloadObs(parcela?.fin_ativos?.obs);
  const entidade = payload?.entidade || null;
  const nomeEntidade = String(entidade?.nome_fantasia || entidade?.nome || '').trim();
  const clienteInstitucional = String(parcela?.fin_ativos?.cliente_nome || '').trim();
  const titulo = nomeEntidade || clienteInstitucional || 'Cobrança não identificada';
  const chave = titulo.toLowerCase();
  const subtitulo = nomeEntidade && clienteInstitucional && nomeEntidade.toLowerCase() !== clienteInstitucional.toLowerCase()
    ? clienteInstitucional
    : '';
  return { chave, titulo, subtitulo, entidade, clienteInstitucional };
}

function renderPainelClientesFinanceiros() {
  const el = document.getElementById('clientefin-lista');
  if (!el) return;

  const q = (document.getElementById('clientefin-q')?.value || '').trim().toLowerCase();
  const hoje = new Date();
  hoje.setHours(0, 0, 0, 0);

  const grupos = new Map();
  (G.parcelas || []).forEach(p => {
    const entidadeData = _clienteFinEntidadeData(p);
    const cliente = entidadeData.titulo;
    const key = entidadeData.chave;
    if (!grupos.has(key)) {
      grupos.set(key, {
        key,
        cliente,
        clienteInstitucional: entidadeData.clienteInstitucional,
        subtitulo: entidadeData.subtitulo,
    ...
    const situacao = String(p.situacao || '').toLowerCase();
    const projetoRef = p.fin_ativos?.num_legado || p.ativo_id || null;
    if (projetoRef) g.projetos.add(String(projetoRef));
    g.total += valor;

    if (situacao === 'pago') {
      g.recebido += valor;
      const prazo = _clienteFinDaysBetween(p.data_emissao_nf || p.data_prevista_cobranca, p.data_pagamento);
      if (prazo !== null) g.prazoDias.push(prazo);
      return;
    }

    if (['entregue', 'a_emitir'].includes(situacao)) {
      g.aEmitir += valor;
      g.futuro += valor;
      g.itensAbertos.push({ ...p, grupoSituacao: 'emitir' });
      return;
    }
    ...
    if (situacao === 'aguardando_pagamento') {
      g.aberto += valor;
      g.nfsAbertas += 1;
      const dtVenc = p.data_vencimento ? new Date(String(p.data_vencimento).slice(0,10) + 'T12:00:00') : null;
      if (dtVenc && !Number.isNaN(dtVenc.getTime()) && dtVenc < hoje) g.vencido += valor;
      else g.futuro += valor;
      g.itensAbertos.push({ ...p, grupoSituacao: 'aguardando' });
    }
  });
  ...
  if (q) {
    lista = lista.filter(g =>
      ...
      String(g.clienteInstitucional || '').toLowerCase().includes(q) ||
      String(g.entidade?.cnpj || '').toLowerCase().includes(q) ||
      [...g.projetos].some(p => String(p).toLowerCase().includes(q))
    );
  }
  lista.sort((a, b) => (b.aberto + b.aEmitir + b.vencido) - (a.aberto + a.aEmitir + a.vencido));

  if (!lista.length) {
    el.innerHTML = `<div class="empty-state">
      <div class="empty-state-icon">📂</div>
      <div class="empty-state-title">Nenhum cliente encontrado</div>
      <div class="empty-state-sub">Ajuste a busca ou aguarde a carga das parcelas de cobrança.</div>
    </div>`;
    return;
  }

  el.innerHTML = lista.map(g => {
    const prazoMedio = g.prazoDias.length ? Math.round(g.prazoDias.reduce((s, n) => s + n, 0) / g.prazoDias.length) : null;
    const projetosLabel = g.projetos.size ? `${g.projetos.size} projeto${g.projetos.size !== 1 ? 's' : ''}` : 'sem projeto vinculado';
    const subtituloParts = [
      g.subtitulo ? `Cliente institucional: ${g.subtitulo}` : '',
      g.entidade?.cnpj ? `CNPJ: ${g.entidade.cnpj}` : '',
      g.nfsAbertas + ` NF${g.nfsAbertas !== 1 ? 's' : ''} em aberto`,
      `Prazo médio: ${prazoMedio === null ? '—' : prazoMedio + ' dia' + (prazoMedio !== 1 ? 's' : '')}`
    ].filter(Boolean);
    const itens = g.itensAbertos
      .sort((a, b) => {
        const da = a.data_vencimento || a.data_prevista_cobranca || '9999-12-31';
        const db = b.data_vencimento || b.data_prevista_cobranca || '9999-12-31';
        return String(da).localeCompare(String(db));
      })
      .slice(0, 3);

    return `
      <div class="clientefin-card">
        <div class="clientefin-head">
          <div>
            <div class="clientefin-title">${_finEsc(g.cliente)}</div>
            <div class="clientefin-meta">
              <span>${_finEsc(projetosLabel)}</span>
              ${subtituloParts.map(item => `<span>${_finEsc(item)}</span>`).join('')}
            </div>
            ${g.entidade?.regra_pagamento ? `<div style="font-size:11px;color:#666;margin-top:8px"><strong>Prática de pagamento:</strong> ${_finEsc(g.entidade.regra_pagamento)}</div>` : ''}
          </div>
          <div class="clientefin-openval">
            <strong>${moeda(g.total)}</strong>
            <span>entrada total da cobrança</span>
          </div>
        </div>

        <div class="clientefin-kpis">
          <div class="clientefin-kpi">
            <div class="clientefin-kpi-lbl">Recebido</div>
            <div class="clientefin-kpi-val">${moeda(g.recebido)}</div>
            <div class="clientefin-kpi-sub">pagamentos confirmados</div>
          </div>
          <div class="clientefin-kpi">
            <div class="clientefin-kpi-lbl">Em aberto</div>
            <div class="clientefin-kpi-val">${moeda(g.aberto)}</div>
            <div class="clientefin-kpi-sub">NFs aguardando pagamento</div>
          </div>
          <div class="clientefin-kpi">
            <div class="clientefin-kpi-lbl">A emitir</div>
            <div class="clientefin-kpi-val">${moeda(g.aEmitir)}</div>
            <div class="clientefin-kpi-sub">parcelas sem NF emitida</div>
          </div>
          <div class="clientefin-kpi">
            <div class="clientefin-kpi-lbl">Vencido</div>
            <div class="clientefin-kpi-val ${g.vencido > 0 ? 'clientefin-risk' : ''}">${moeda(g.vencido)}</div>
            <div class="clientefin-kpi-sub">cobranças atrasadas</div>
          </div>
          <div class="clientefin-kpi">
            <div class="clientefin-kpi-lbl">Futuro</div>
            <div class="clientefin-kpi-val ${g.futuro > 0 ? 'clientefin-ok' : ''}">${moeda(g.futuro)}</div>
            <div class="clientefin-kpi-sub">a emitir ou a receber</div>
          </div>
        </div>

        <div class="clientefin-openlist">
          ${itens.length ? itens.map(p => {
            const tipoDisp = _finEsc(TIPO_PARCELA_MAP[p.tipo]?.label || p.tipo || 'Parcela');
            const descDisp = _finEsc(p.descricao || 'Parcela sem descrição');
            const dataRef = p.grupoSituacao === 'aguardando'
              ? `Vence ${fmtData(p.data_vencimento) || '—'}`
              : `Prev. ${fmtData(p.data_prevista_cobranca) || '—'}`;
            const projetoDisp = _finEsc(p.fin_ativos?.num_legado || 'sem projeto');
            return `
              <div class="clientefin-openrow">
                <div class="clientefin-openinfo">
                  <div class="clientefin-openmain">${tipoDisp} · ${descDisp}</div>
                  <div class="clientefin-opensub">${projetoDisp} · ${dataRef}</div>
                </div>
                <div class="clientefin-openval">
                  <strong>${moeda(parseFloat(p.valor) || 0)}</strong>
                  <span>${p.grupoSituacao === 'aguardando' ? 'NF em aberto' : 'aguardando emissão'}</span>
                </div>
              </div>`;
          }).join('') : `<div class="empty-state"><div class="empty-state-title">Nenhuma pendência aberta</div><div class="empty-state-sub">Este cliente não tem parcelas abertas ou aguardando emissão.</div></div>`}
        </div>
      </div>`;
  }).join('');
}
...
async function carregarPipeline() {
  ...
  let q = sb.from('fin_parcelas')
    .select('*, fin_ativos(cliente_nome, num_legado, nucleo, obs)')
    .in('situacao', ['entregue','a_emitir','aguardando_pagamento','pago'])
    .order('data_prevista_cobranca');

  const { data: all } = await q;
  G.parcelas = all || [];
  renderPainelClientesFinanceiros();
...
function kanbanCard(p, colTipo) {
  ...
  return `<div class="kanban-card" onclick="abrirParcelaModal(${parcelaIdJs})">
    <div class="kc-tipo">
      ...
      ${tipoDisp}
      ${nc.cls ? `· <span class="badge ${nc.cls}" style="padding:1px 5px">${nucleoLabelDisp}</span>` : ''}
    </div>
    <div class="kc-cliente">${clienteDisp}</div>
    <div class="kc-projeto">${legadoDisp} ${p.descricao ? '· ' + descDisp : ''}</div>
    <div class="kc-valor">${moeda(p.valor)}</div>
    <div class="kc-meta">
      ${colTipo === 'emitir' ? `Previsto: ${fmtData(p.data_prevista_cobranca)}` : ''}
      ${colTipo === 'aguardando' ? `Vence: ${fmtData(p.data_vencimento)||'—'}` : ''}
      ${colTipo === 'pago' ? `Pago em: ${fmtData(p.data_pagamento)}` : ''}
      ${vencido ? `<span style="color:var(--terracota);font-weight:700"> · ${dias}d atrasado</span>` : ''}
    </div>
...
async function emitirParcela(id) {
  const nf = prompt('Número da NF (deixe em branco se ainda não tem):');
  ...
  toast('Parcela marcada como emitida ✓', 'ok');
  carregarPipeline();
}
...
function confirmarPagamento(id) {
  ...
  document.getElementById('pgto-desc-label').textContent =
    [p?.fin_ativos?.num_legado, p?.descricao].filter(Boolean).join(' · ') || '—';
...
async function salvarConfirmacaoPgto() {
  ...
  // 2. Baixa em fin_lancamentos — tenta atualizar lançamento existente ligado à parcela
  ...
  if (lancExist?.length) {
    ...
    // Cria o lançamento de receita se não existia
    ...
    await sb.from('fin_lancamentos').insert({
      ...
      observacao: obs || [p.fin_ativos?.num_legado, p.descricao].filter(Boolean).join(' · ') || 'Pagamento recebido',
      valor: p.valor,
      tipo: 'receita',
      situacao: 'pago',
      origem: 'crm',
  ...
  toast('Pagamento confirmado e baixa realizada ✓', 'ok');
...
function abrirParcelaModal(id) {
  ...
  document.getElementById('modal-parcela-titulo').textContent =
    `${tp.label || p.tipo} · ${p.fin_ativos?.cliente_nome || '—'}`;
  document.getElementById('modal-parcela-body').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      ...
      <div><div style="font-size:9px;font-weight:700;color:#888">SITUAÇÃO</div><div>${sitBadge(p.situacao)}</div></div>
      ...
      <div><div style="font-size:9px;font-weight:700;color:#888">%</div><div>${p.percentual ? p.percentual + '%' : '—'}</div></div>
      <div><div style="font-size:9px;font-weight:700;color:#888">PREV. COBRANÇA</div><div>${fmtData(p.data_prevista_cobranca)}</div></div>
      <div><div style="font-size:9px;font-weight:700;color:#888">NF</div><div>${p.numero_nf||'—'}</div></div>
      <div><div style="font-size:9px;font-weight:700;color:#888">EMISSÃO NF</div><div>${fmtData(p.data_emissao_nf)}</div></div>
...
async function carregarLancamentos() {
  ...
  G.lancamentos = data || [];
  await carregarBhPagamentosPendentes();
...
async function carregarBhPagamentosPendentes() {
  const wrap = document.getElementById('lanc-bh-pendentes');
  const count = document.getElementById('lanc-bh-count');
  if (!wrap || !count) return;
  try {
    const { data, error } = await sb.from('banco_horas_solicitacoes')
      .select('*')
      .eq('status_destino', 'aguardando_pagamento_extraordinario')
      .gt('horas_para_pagamento_h', 0)
    ...
    let lancamentos = [];
    if (ids.length) {
      const linked = await sb.from('fin_lancamentos')
        .select('id, origem_id, situacao, valor, data_competencia, data_pagamento, fornecedor_cliente')
        .eq('origem', 'pagamento_extraordinario')
        .in('origem_id', ids);
      if (linked.error) throw linked.error;
      lancamentos = linked.data || [];
    }
    const byOrigem = new Map(lancamentos.map(l => [String(l.origem_id || ''), l]));
    G.bhPagamentosPendentes = rows.map(r => ({
      ...r,
      lancamentoFinanceiro: byOrigem.get(String(r.id || '')) || null,
    }));
    renderBhPagamentosPendentes();
  } catch (error) {
    G.bhPagamentosPendentes = [];
    count.textContent = 'Indisponível';
    wrap.innerHTML = `<div class="tbl-empty" style="padding:6px 0">Não foi possível carregar os pagamentos extraordinários: ${_finEsc(error.message || 'erro desconhecido')}</div>`;
  }
}

function renderBhPagamentosPendentes() {
  const wrap = document.getElementById('lanc-bh-pendentes');
  const count = document.getElementById('lanc-bh-count');
  if (!wrap || !count) return;
  const lista = G.bhPagamentosPendentes || [];
  count.textContent = lista.length
    ? `${lista.length} pendência${lista.length > 1 ? 's' : ''} em aberto`
    : 'Nenhuma pendência em aberto';
  if (!lista.length) {
    wrap.innerHTML = '<div class="tbl-empty" style="padding:6px 0">Nenhum pagamento extraordinário pendente.</div>';
    return;
  }
  wrap.innerHTML = lista.map(item => {
    const lanc = item.lancamentoFinanceiro;
    const horas = Number(item.horas_para_pagamento_h || 0);
    const statusLanc = lanc ? sitBadge(lanc.situacao || 'a_pagar') : '<span class="badge b-am">Sem lançamento</span>';
    const acao = lanc
      ? `<button class="btn btn-sec btn-sm" onclick="editarLancamento(${_finSq(lanc.id)})">Editar lançamento</button>`
      : `<button class="btn btn-pri btn-sm" onclick="abrirPagamentoExtraDaSolicitacao(${_finSq(item.id)})">Gerar lançamento</button>`;
    return `
      <div class="card card-sm" style="padding:12px">
        <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap">
          <div style="min-width:260px;flex:1">
            <div style="font-size:13px;font-weight:700">${_finEsc(item.usuario_nome || 'Colaborador')}</div>
            <div style="font-size:11px;color:#666;margin-top:2px">
              Semana ${_finEsc(fmtData(item.semana_inicio))} — ${_finEsc(fmtData(item.semana_fim))}
              · ${_finEsc(String(horas))}h aprovadas para pagamento
            </div>
            <div style="font-size:11px;color:#666;margin-top:4px">
              ${statusLanc}
              <span style="margin-left:6px">${_finEsc(item.aprovado_por_nome || 'Aprovação registrada')}</span>
            </div>
            ${item.observacao_decisao ? `<div style="font-size:11px;color:#666;margin-top:6px"><strong>Decisão:</strong> ${_finEsc(item.observacao_decisao)}</div>` : ''}
            ${lanc ? `<div style="font-size:11px;color:#666;margin-top:6px"><strong>Lançamento:</strong> ${_finEsc(lanc.fornecedor_cliente || 'Pagamento extraordinário')} · ${moeda(lanc.valor, true)} · competência ${_finEsc(fmtMesAno(lanc.data_competencia))}</div>` : ''}
          </div>
...
function abrirPagamentoExtraDaSolicitacao(id) {
  const item = (G.bhPagamentosPendentes || []).find(x => String(x.id) === String(id));
  if (!item) {
    toast('Solicitação de pagamento extraordinário não encontrada.', 'erro');
    return;
  }
  if (item.lancamentoFinanceiro && item.lancamentoFinanceiro.id) {
    editarLancamento(item.lancamentoFinanceiro.id);
    return;
  }
  abrirModalPagamentoExtraordinario();
  const competencia = item.semana_fim || new Date().toISOString().slice(0,10);
  const valorField = document.getElementById('lanc-valor');
  const desc = document.getElementById('lanc-descricao');
  const comp = document.getElementById('lanc-competencia');
  const prev = document.getElementById('lanc-prevista');
  const ref = document.getElementById('lanc-origem-ref');
  if (comp) comp.value = competencia;
  if (prev) prev.value = competencia;
  if (ref) ref.value = item.id;
  if (desc) desc.value = `Pagamento extraordinário de ${item.horas_para_pagamento_h || 0}h - ${item.usuario_nome || 'Colaborador'}`;
  if (valorField && !valorField.value) valorField.placeholder = 'Informe o valor final a pagar';
}

async function sincronizarPagamentoExtraordinario(origemId, situacaoNova, situacaoAnterior) {
  if (!origemId) return;
  const { data: req, error } = await sb.from('banco_horas_solicitacoes')
    .select('id, usuario_id, horas_para_pagamento_h, status_destino')
    .eq('id', origemId)
    .maybeSingle();
  if (error) throw error;
  if (!req) return;
  const horas = Number(req.horas_para_pagamento_h || 0);
  if (!horas) return;

  if (situacaoNova === 'pago' && situacaoAnterior !== 'pago') {
    const saldoRes = await sb.from('banco_horas_saldo').select('horas_pagamento_pendente, horas_pagas').eq('usuario_id', req.usuario_id).maybeSingle();
    if (saldoRes.error && saldoRes.error.code !== 'PGRST116') throw saldoRes.error;
    const saldoAtual = saldoRes.data || { horas_pagamento_pendente: 0, horas_pagas: 0 };
    const pend = Math.max(0, Number(saldoAtual.horas_pagamento_pendente || 0) - horas);
    const pagas = Number(saldoAtual.horas_pagas || 0) + horas;
    const updSaldo = await sb.from('banco_horas_saldo').upsert({
      usuario_id: req.usuario_id,
      horas_pagamento_pendente: pend,
      horas_pagas: pagas,
      updated_at: new Date().toISOString(),
    }, { onConflict: 'usuario_id' });
    if (updSaldo.error) throw updSaldo.error;
    const updReq = await sb.from('banco_horas_solicitacoes').update({
  ...
  if (situacaoNova !== 'pago' && situacaoAnterior === 'pago') {
    ...
    if (saldoRes.error && saldoRes.error.code !== 'PGRST116') throw saldoRes.error;
    const saldoAtual = saldoRes.data || { horas_pagamento_pendente: 0, horas_pagas: 0 };
    const pend = Number(saldoAtual.horas_pagamento_pendente || 0) + horas;
    const pagas = Math.max(0, Number(saldoAtual.horas_pagas || 0) - horas);
    const updSaldo = await sb.from('banco_horas_saldo').upsert({
      usuario_id: req.usuario_id,
      horas_pagamento_pendente: pend,
      horas_pagas: pagas,
    ...
    if (updSaldo.error) throw updSaldo.error;
    const updReq = await sb.from('banco_horas_solicitacoes').update({
      status_destino: 'aguardando_pagamento_extraordinario',
      updated_at: new Date().toISOString(),
    }).eq('id', origemId);
    if (updReq.error) throw updReq.error;
  }
...
function filtrarLancamentos() {
  ...
  if (!lista.length) {
    tbody.innerHTML = '<tr><td colspan="9" class="tbl-empty">Nenhum lançamento encontrado</td></tr>';
    return;
  }
  tbody.innerHTML = lista.map(l => {
    ...
    const principalDisp = _finEsc(l.fornecedor_cliente||l.observacao||'—');
    const obsDisp = _finEsc(l.observacao||'');
    const subcontaDisp = _finEsc(l.subconta||l.grupo||'—');
    const nfDisp = _finEsc(l.nf||'—');
    return `
    <tr${bloq ? ' style="opacity:.75"' : ''}>
      <td>${fmtMesAno(l.data_competencia)}${bloq ? ' <span title="Mês fechado" style="font-size:10px">🔒</span>' : ''}</td>
      ...
      <td>${bloq
        ? '<span title="Mês fechado — edição bloqueada" style="font-size:14px;cursor:default">🔒</span>'
        : `<button class="btn btn-ghost btn-xs" onclick="editarLancamento(${lancIdJs})">✏</button>`
      }</td>
    </tr>`;
  }).join('');
}
...
function abrirModalLancamento(id) {
  ...
  _lancEditId = id || null;
  document.getElementById('modal-lanc-titulo').textContent = id ? 'Editar Lançamento' : 'Novo Lançamento';
  ...
  if (id) {
    ...
    if (l) {
      ...
      if (_lancOrigemMode === 'pagamento_extraordinario') {
        document.getElementById('modal-lanc-titulo').textContent = 'Editar Pagamento Extraordinário';
        if (banner) {
          banner.style.display = 'block';
          banner.textContent = 'Este lançamento está marcado como pagamento extraordinário de horas adicionais.';
        }
...
function abrirModalPagamentoExtraordinario() {
  ...
  document.getElementById('modal-lanc-titulo').textContent = 'Novo Pagamento Extraordinário';
  const banner = document.getElementById('lanc-origem-banner');
  if (banner) {
    banner.style.display = 'block';
    banner.textContent = 'Use esta entrada apenas para horas adicionais já aprovadas para pagamento. Registre a saída com valor negativo.';
  }
  const refRow = document.getElementById('lanc-ref-row');
  if (refRow) refRow.style.display = '';
  const sit = document.getElementById('lanc-situacao-form');
  ...
  const desc = document.getElementById('lanc-descricao');
  if (desc) desc.placeholder = 'Ex: Pagamento extraordinário de horas adicionais - Colaborador X';
}
...
async function salvarLancamento() {
  ...
  if (!competencia || !descricao || isNaN(valor) || !plano) {
    toast('Preencha os campos obrigatórios', 'erro'); return;
  }
  // Bloquear mês fechado
  const [y, m] = competencia.split('-');
  const mesAnoKey = m + y;
  if (G._mesesFechados.has(mesAnoKey)) {
    toast('Este mês está fechado. Reabra-o antes de editar.', 'erro'); return;
  }
  const payload = {
  ...
  const situacaoNova = payload.situacao || '';
  const lancAnterior = _lancEditId ? (G.lancamentos || []).find(x => x.id === _lancEditId) : null;
  const situacaoAnterior = lancAnterior ? (lancAnterior.situacao || '') : '';
  if (_lancOrigemMode === 'pagamento_extraordinario' && origemRef) {
    const dup = await sb.from('fin_lancamentos')
      .select('id')
      .eq('origem', 'pagamento_extraordinario')
      .eq('origem_id', origemRef)
      .limit(2);
    if (dup.error) { toast('Erro ao validar vínculo do pagamento: ' + dup.error.message, 'erro'); return; }
    const duplicado = (dup.data || []).find(x => !_lancEditId || x.id !== _lancEditId);
    if (duplicado) {
      toast('Esta aprovação já possui um lançamento financeiro vinculado.', 'erro');
      return;
    }
  }
  let error;
  ...
  if (_lancOrigemMode === 'pagamento_extraordinario' && origemRef) {
    try {
      await sincronizarPagamentoExtraordinario(origemRef, situacaoNova, situacaoAnterior);
    } catch (syncError) {
      toast('Lançamento salvo, mas a sincronização com banco de horas falhou: ' + syncError.message, 'erro');
    }
  }
  toast(_lancEditId ? 'Lançamento atualizado ✓' : 'Lançamento salvo ✓', 'ok');
  _lancEditId = null;
...
//   CONCILIAÇÃO
// ══════════════════════════════════════════════════════════
function abrirImportExtrato() { toast('Importação de extrato em breve…'); }
...
function renderFechamentos() {
  ...
  // Gerar lista de meses desde Jan/2026 até mês atual
  const hoje = new Date();
  ...
  if (!meses.length) {
    el.innerHTML = '<div class="empty-state"><div class="empty-state-title">Nenhum mês disponível</div></div>';
    return;
  }
  ...
  el.innerHTML = meses.map(({ ano, mes }) => {
    ...
    if (fech) {
      const snapItens = [
        ...
        { lbl: 'Rec. Líquida', val: fech.snap_receita_liq },
        { lbl: 'Lucro Bruto',  val: fech.snap_lucro_bruto },
        { lbl: 'Res. Op.',     val: fech.snap_resultado_op },
        { lbl: 'Res. Líquido', val: fech.snap_resultado_liq },
        { lbl: 'Margem Líq.',  val: fech.snap_margem_liq, pct: true },
      ].map(s => `
        <div class="fech-snap-item">
          ...
          <div class="fech-snap-val" style="color:${s.pct ? (s.val>=0?'var(--verde)':'var(--terracota)') : (s.val>=0?'var(--grafite)':'var(--terracota)')}">${
            s.pct ? (s.val != null ? s.val.toFixed(1)+'%' : '—') : moeda(s.val, true)
          }</div>
        </div>`).join('');

      const fechadoPor = fech.fechado_por_nome ? ` · por ${_finEsc(fech.fechado_por_nome)}` : '';
      const fechadoEm  = fech.created_at ? ' · ' + fmtData(fech.created_at.slice(0,10)) : '';
      const notasDisp = _finEsc(fech.notas || '');

      return `
      <div class="fech-row fechado">
        <span class="fech-lock">🔒</span>
      ...
      <div class="fech-row${isAtual ? ' atual' : ''}">
        <span class="fech-lock" style="opacity:.3">🔓</span>
        <div class="fech-mes">${nomeMesDisp}${isAtual ? ' <span style="font-size:9px;color:var(--azul);font-weight:400">atual</span>' : ''}</div>
        <div class="fech-snap" style="color:#aaa;font-size:11px;align-items:center">Mês em aberto</div>
        <div class="fech-meta"></div>
        <div class="fech-actions">
          ${G.isSocio ? `<button class="btn btn-pri btn-sm" onclick="abrirFecharMes(${ano},${mes})">Fechar mês</button>` : ''}
        </div>
      </div>`;
    }
  }).join('');
...
// Calcula snapshot DRE para um mês específico (retorna objeto snap)
async function calcSnapMes(ano, mes) {
  ...
  const deducoes  = get('Deduções');
  const recLiq    = recBruta - deducoes;
  const custos    = get('Custos Diretos', 'Custo de Venda');
  const lucroBruto = recLiq - custos;
  const despOp    = get('Despesas com Pessoal','Despesas Administrativas','Despesas com Operação','Despesas com Marketing','Outras Despesas');
  const resOp     = lucroBruto - despOp;
  const despFin   = get('Investimentos','Despesa Financeira','Depreciação','CAPEX');
...
let G_fechMes = null; // contexto do mês sendo fechado

async function abrirFecharMes(ano, mes) {
  if (!G.isSocio) return;
  const nomeMes = MESES_FULL[mes - 1] + ' ' + ano;
  document.getElementById('fech-modal-titulo').textContent = `Fechar ${nomeMes}`;
  document.getElementById('fech-modal-desc').textContent = `Calculando resultado de ${nomeMes}…`;
  document.getElementById('fech-preview-dre').innerHTML = '<div style="text-align:center;color:#aaa;padding:20px">Carregando…</div>';
  ...
  document.getElementById('fech-modal-desc').textContent =
    `Ao fechar ${nomeMes}, todos os lançamentos do período ficam bloqueados. O snapshot abaixo é salvo para histórico.`;

  const linhas = [
    ['Receita Bruta',         snap.snap_receita_bruta,  false],
    ['Receita Líquida',       snap.snap_receita_liq,    false],
    ['Lucro Bruto',           snap.snap_lucro_bruto,    false],
    ['Resultado Operacional', snap.snap_resultado_op,   false],
    ['Resultado Líquido',     snap.snap_resultado_liq,  false],
    ['Margem Líquida',        snap.snap_margem_liq,     true ],
      ...
      <div class="pc-grupo-body open" id="grp-${grp.replace(/[^a-z0-9]/gi,'-')}">
        ${items.map(p => `
          <div class="pc-item">
            ...
            ${G.isSocio ? `<button class="btn btn-ghost btn-xs" onclick="editarSubconta(${_finSq(p.id)})">✏</button>` : ''}
          </div>`).join('')}
      </div>
    </div>`).join('');
}
...
async function salvarSubconta() {
  ...
  toast(_subEditId ? 'Subconta atualizada ✓' : 'Subconta criada ✓', 'ok');
  _subEditId = null;
...
function renderContas() {
  ...
  const ICON = { corrente:'🏦', aplicacao:'📈', investimento:'💰' };
  ...
  el.innerHTML = G.contasBancarias.map(c => `
    <div class="conta-card">
      <div class="conta-icon">${ICON[c.tipo]||'🏦'}</div>
      <div class="conta-info">
        <div class="conta-nome">${_finEsc(c.nome)}</div>
        <div class="conta-meta">${_finEsc(c.banco||'')} · ${_finEsc(c.tipo)}</div>
      </div>
      <div class="conta-saldo">
        <div class="conta-saldo-val">${moeda(c.saldo_inicial, true)}</div>
        <div class="conta-saldo-lbl">saldo inicial</div>
      </div>
      ${G.isSocio ? `<button class="btn btn-ghost btn-sm" onclick="editarConta(${_finSq(c.id)})">✏</button>` : ''}
    </div>`).join('');
}
...
function abrirModalConta(id) {
  ...
  document.getElementById('modal-conta-titulo').textContent = id ? 'Editar Conta' : 'Nova Conta Bancária';
...
async function salvarConta() {
  ...
  if (error) { toast('Erro: ' + error.message, 'erro'); return; }
  toast(_contaEditId ? 'Conta atualizada ✓' : 'Conta criada ✓', 'ok');
  _contaEditId = null;
...
function finSetEntidadeCobrancaFields(entidade) {
  ...
  document.getElementById('fin-cob-regra').value = data.regra_pagamento || '';
}
...
function finGetEntidadeCobrancaFields() {
  return {
    ...
    regra_pagamento: document.getElementById('fin-cob-regra').value.trim() || null
  };
}
...
async function finalizarRascunho(id) {
  ...
  if (!ativo) { toast('Ativo não encontrado', 'erro'); return; }
  ...
  const subtit = [ativo.num_legado, ativo.prop_code].filter(Boolean).join(' · ');
  ...
  document.getElementById('fin-num-legado').textContent = ativo.descricao || ativo.num_legado || ativo.prop_code || '—';
...
function renderEtapasFinalizacao() {
  ...
  if (!etapas.length) {
    el.innerHTML = `<div style="text-align:center;padding:20px;color:#bbb;font-size:11px;border:1px dashed var(--cinza);border-radius:8px">
      Nenhuma etapa pré-preenchida. Adicione etapas manualmente.
    </div>`;
    return;
  }
  ...
  el.innerHTML = etapas.map((e, ei) => {
    ...
    return `<div style="border:1px solid var(--cinza);border-radius:8px;margin-bottom:10px;overflow:hidden" id="etapa-blk-${ei}">
      ...
      <div style="padding:10px 14px" id="parcelas-blk-${ei}">
        ...
        ${!parcelas.length ? `<div style="font-size:10px;color:#bbb;padding:4px 0">Nenhuma parcela — clique em "+ Parcela" para adicionar</div>` : ''}
      </div>
    </div>`;
  }).join('');
  atualizarSomaCheck();
...
function parcelaFormHTML(ei, pi, p) {
  ...
  return `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--cinza2)" id="parcela-row-${ei}-${pi}">
    ...
    <input type="text" value="${descDisp}" placeholder="Descrição (opcional)"
    ...
    <button class="btn btn-ghost btn-xs" style="color:var(--terracota)" onclick="removerParcela(${ei},${pi})">✕</button>
  </div>`;
}
...
function adicionarRepasse() { toast('Adicionar repasse manual em breve…'); }
...
async function confirmarAtivo() {
  ...
  // Mapear plano_contas de receita pelo núcleo do ativo
  ...
  // Mapear plano_contas de custos para subcontratações e repasses
  ...
  if (mod === 'etapas') {
    ...
    if (Math.abs(soma - 100) > 0.5) {
      ...
      el.style.display = '';
      el.textContent = `⚠ A soma das parcelas é ${soma.toFixed(1)}%. Deve ser 100% para confirmar.`;
      return;
    }
  }
  ...
  // Criar fin_lancamentos para subcontratações e repasses (custos do projeto)
  const hoje = new Date().toISOString().split('T')[0];
  ...
  if (subs?.length && pcSub) {
    for (const sub of subs) {
      ...
      await sb.from('fin_lancamentos').insert({
        ...
        observacao: `Subcontratação: ${sub.servico || sub.empresa || 'Serviço ext.'}`,
  ...
  toast('Projeto ativado com sucesso! ✓', 'ok');
...
async function excluirRascunho() {
  if (!G_ativoEdit) return;
  if (!confirm(`Excluir rascunho de "${G_ativoEdit.cliente_nome}"? Esta ação não pode ser desfeita.`)) return;
...
const DRE_ESTRUTURA = [
  ...
  { t: 'grupo',    key: 'Deduções',                   label: '(−) Deduções da Receita',   sinal: -1 },
  { t: 'subtotal', key: 'rec_liq',   label: 'Receita Líquida',      linha: true },
  { t: 'secao',    label: 'CUSTOS' },
  { t: 'grupo',    key: 'Custos Diretos',             label: '(−) Custos Diretos',        sinal: -1 },
  { t: 'grupo',    key: 'Custo de Venda',             label: '(−) Custo de Venda',        sinal: -1 },
  ...
  { t: 'grupo',    key: 'Despesas com Pessoal',       label: '(−) Pessoal',               sinal: -1 },
  { t: 'grupo',    key: 'Despesas Administrativas',   label: '(−) Administrativas',       sinal: -1 },
  { t: 'grupo',    key: 'Despesas com Operação',      label: '(−) Operação',              sinal: -1 },
  { t: 'grupo',    key: 'Despesas com Marketing',     label: '(−) Marketing',             sinal: -1 },
  { t: 'grupo',    key: 'Outras Despesas',            label: '(−) Outras',                sinal: -1 },
  ...
  { t: 'grupo',    key: 'Investimentos',              label: '(−) Investimentos',         sinal: -1 },
  { t: 'grupo',    key: 'Despesa Financeira',         label: '(−) Desp. Financeiras',     sinal: -1 },
  { t: 'grupo',    key: 'Depreciação',                label: '(−) Depreciação',           sinal: -1 },
  { t: 'grupo',    key: 'CAPEX',                      label: '(−) CAPEX',                 sinal: -1 },
  { t: 'subtotal', key: 'res_liq',  label: 'RESULTADO LÍQUIDO',    linha: true, destaque: true },
  { t: 'margem',   key: 'mg_liq',   label: 'Margem Efetiva',        destaque: true },
];

// Mapeamento de subtotais: quais grupos somam para cada chave
const DRE_SUBTOTAIS = {
  rec_liq:    { grupos_pos: ['Receita Bruta','Receita Financeira'], grupos_neg: ['Deduções'] },
  lucro_bruto:{ base: 'rec_liq',     grupos_neg: ['Custos Diretos','Custo de Venda'] },
  res_op:     { base: 'lucro_bruto', grupos_neg: ['Despesas com Pessoal','Despesas Administrativas','Despesas com Operação','Despesas com Marketing','Outras Despesas'] },
  res_liq:    { base: 'res_op',      grupos_neg: ['Investimentos','Despesa Financeira','Depreciação','CAPEX'] },
  // margens
...
async function carregarDRE() {
  ...
  if (container) container.innerHTML = '<div class="empty-state">Carregando…</div>';
...
// Agrega lançamentos por grupo e por periodo (mês 1-12 ou quadrimestre 1-3)
...
function dreDD(idx) {
  ...
  // Rótulo do período
  let periodoLabel = `${ano} · Ano todo`;
  if (periodo !== null) {
    if (modo === 'anual') periodoLabel = MESES_FULL[periodo - 1] + ' · ' + ano;
    else periodoLabel = `Q${periodo} · ${QUAD[periodo - 1]?.label} · ${ano}`;
  }
  ...
  const tituloBase = item.tipo === 'subconta'
    ? item.key
    : (estrutItem?.label?.replace(/^\(−\) /, '') || item.key);

  document.getElementById('drd-titulo').textContent = tituloBase;
  document.getElementById('drd-sub').textContent    = `${periodoLabel} · ${dados.length} lançamento${dados.length !== 1 ? 's' : ''}`;
  ...
  const tbRows = dados.length
    ? dados.map(l => {
      ...
      <tr class="dd-total">
        <td colspan="5">Total (${dados.length} lançamento${dados.length !== 1 ? 's' : ''})</td>
        <td style="text-align:right;color:${total >= 0 ? 'var(--verde)' : 'var(--terracota)'}">${moeda(total)}</td>
      </tr>`
    : '<tr><td colspan="6" style="text-align:center;color:#aaa;padding:28px 0">Nenhum lançamento no período</td></tr>';
...
function renderDRE() {
  const container = document.getElementById('dre-container');
  if (!container) return;
  if (!G_dre.dados) { container.innerHTML = '<div class="empty-state">Carregando…</div>'; return; }
  ...
  // Filtro por núcleo: mantém lançamentos do núcleo selecionado E despesas (nucleo_crm nulo)
  ...
  // Helper: formata célula de valor DRE
  ...
  // Helper: célula com real+proj quando tipo='ambos'
  const dreCell = (periodoData, isMargem = false) => {
    if (!periodoData) return '<span style="color:#ddd">—</span>';
  ...
  // Helper: célula de desvio (real - orçado), com sinal favorável dependendo do tipo do item
  const dreDesvio = (realVal, projVal, sinal) => {
    if (realVal === null || realVal === undefined || projVal === null || projVal === undefined) {
      return '<span class="dre-desvio-zero">—</span>';
    }
    const desvio = (realVal - projVal) * sinal; // positivo = melhor que o orçado
    if (Math.abs(desvio) < 1) return '<span class="dre-desvio-zero">—</span>';
  ...
  const thCols = colunas.map(c =>
    ...
    + (mostrarDesvio ? '<th class="col-total" style="min-width:80px">Orçado</th><th class="col-desvio">Desvio</th>' : '');
  ...
  for (const item of DRE_ESTRUTURA) {
    ...
    if (item.t === 'grupo') {
      ...
      // Células por período — cada uma tem seu próprio item de drill
      ...
      rows += `<tr${!hasData ? ' style="opacity:.4"' : ''}>
        <td class="dre-label-click" style="padding-left:20px" onclick="dreDD(${ddIdx})">${item.label}<span class="dre-dd-hint">↗</span></td>
        ${tdCols}
      ...
      if (detalhar && hasData) {
        ...
        for (const sub of subs) {
          ...
          rows += `<tr style="font-size:10px;color:#666">
            <td class="dre-label-click" style="padding-left:36px" onclick="dreDD(${ddSubIdx})">${_finEsc(sub.subconta)}<span class="dre-dd-hint">↗</span></td>
            ${subTdCols}
      ...
      const mgDesvio = mostrarDesvio && tot
        ? `<td class="col-total">${dreV(tot.proj, true)}</td><td class="col-desvio" style="text-align:right">${
            (tot.real !== null && tot.proj !== null)
              ...
              : '<span class="dre-desvio-zero">—</span>'
          }</td>`
        : '';
  ...
  const nucleoLabel = nucleo ? ` · ${NUCLEO_MAP[nucleo]?.label || nucleo}` : '';
  const tipoLabel   = tipo === 'real' ? 'Realizado' : tipo === 'proj' ? 'Projetado' : 'Realizado vs Orçado';
  container.innerHTML = `
    <table class="dre-tbl">
      <thead><tr>
        <th>${G_dre.ano}${nucleoLabel} · ${tipoLabel}</th>
        ${thCols}
      </tr></thead>
      <tbody>${rows}</tbody>
    </table>`;
...
// Quadrimestres fixos
const QUAD = [
  { label: 'Jan – Abr', meses: [1,2,3,4] },
  { label: 'Mai – Ago', meses: [5,6,7,8] },
  { label: 'Set – Dez', meses: [9,10,11,12] },
];
...
async function gerarLancamentosPrevistos() {
  if (G._gerandoPrevisoes) return; // previne execução concorrente
  ...
  for (const rec of recs) {
    for (let i = 0; i < 6; i++) {
      ...
      // Verificar duplicata usando origem_id (texto, sempre disponível no schema)
...
function planValCell(real, prev) {
  ...
  if (!hasReal && !hasPrev) return '<span style="color:#ccc">—</span>';
  let h = '';
...
function renderMesAMes() {
  ...
  if (!container) return;
  if (!G_plan.dados) { container.innerHTML = '<div class="empty-state">Carregando…</div>'; return; }
    ...
    grupos.forEach(g => {
      html += `<tr><td style="padding-left:20px;font-size:11px;color:#555">${g.replace(/_/g,' ')}</td>
        ${rowCells(m => {
          ...
          return c.real > 0 || c.prev > 0 ? planValCell(c.real, c.prev) : '<span style="color:#ddd">·</span>';
        })}</tr>`;
    });
  }
  ...
  html += `<tr class="row-total"><td>Resultado</td>${rowCells(m => {
    ...
    return hasData ? `<span class="${res >= 0 ? 'positivo' : 'negativo'}">${moeda(res)}</span>` : '<span style="color:#ccc">—</span>';
  })}</tr>`;
  html += `<tr class="row-margem"><td>Margem Efetiva</td>${rowCells(m => {
    ...
    if (rec <= 0) return '<span style="color:#ccc">—</span>';
  ...
  QUAD.forEach((q, qi) => {
    ...
    html += `<div class="quad-card${isAtual ? ' atual' : ''}">
      <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:${isAtual ? 'var(--fin)' : '#888'};margin-bottom:8px">
        Q${qi+1} · ${q.label}${isAtual ? ' <span style="background:var(--fin);color:#fff;padding:1px 6px;border-radius:10px;font-size:8px">atual</span>' : ''}
      </div>
      <div class="quad-metric">
        <div style="font-size:10px;color:#888;margin-bottom:2px">Receita</div>
        <div style="font-size:19px;font-weight:700;color:var(--verde)">${receita > 0 ? moeda(receita, true) : '—'}</div>
      ...
      <div class="quad-metric">
        ...
        <div style="font-size:19px;font-weight:700;color:var(--terracota)">${despesa > 0 ? moeda(despesa, true) : '—'}</div>
      ...
      <div class="quad-margens">
        <div class="quad-margem-item">
          ...
          <div class="quad-margem-val ${mgEf !== null ? (mgEf >= 0 ? 'positivo' : 'negativo') : ''}">${mgEf !== null ? fmtPct(mgEf) : '—'}</div>
        </div>
      </div>
    </div>`;
  });
    ...
    html += `<div class="proj-card${isAtual ? ' atual' : ''}">
      ...
      <div class="proj-metricas">
        <div>
          ...
          <div style="font-weight:600;font-size:12px;color:var(--verde)">${receita > 0 ? moeda(receita, true) : '—'}</div>
        </div>
        <div>
          <div style="font-size:9px;color:#888">Despesa</div>
          <div style="font-weight:600;font-size:12px;color:var(--terracota)">${despesa > 0 ? moeda(despesa, true) : '—'}</div>
        </div>
        <div>
          <div style="font-size:9px;color:#888">Margem</div>
          <div style="font-weight:700;font-size:13px;color:${mg !== null ? (mg >= 0 ? 'var(--verde)' : 'var(--terracota)') : '#ccc'}">${mg !== null ? fmtPct(mg) : '—'}</div>
        </div>
      </div>
      ${isFuturo ? '<div style="font-size:9px;color:#9B8DC0;margin-top:6px;text-align:center">▸ projeção</div>' : ''}
    </div>`;
  }
  container.innerHTML = html;
}
...
function renderRecorrentes() {
  ...
  container.innerHTML = recs.map(r => {
    ...
    if (r.tipo_recorrencia === 'sazonal' && r.meses_do_ano?.length) {
      ...
      meta = r.dia_vencimento ? `Todo dia ${r.dia_vencimento}` : 'Todo mês';
    }
    const plano = r.fin_plano_contas ? `${r.fin_plano_contas.grupo} · ${r.fin_plano_contas.subconta}` : '';

    return `<div class="rec-item">
      <div class="rec-info">
        <div class="rec-nome">${r.descricao}</div>
        <div class="rec-meta">${[plano, meta, r.obs].filter(Boolean).join(' · ')}</div>
      </div>
...
async function salvarRecorrente() {
  ...
  if (!desc || isNaN(valor) || valor <= 0 || !inicio) {
    toast('Preencha descrição, valor e data início'); return;
  }

  let meses = null;
  if (tipo === 'sazonal') {
    meses = [...document.querySelectorAll('.rec-mes-chk:checked')].map(c => parseInt(c.value));
    if (!meses.length) { toast('Selecione ao menos um mês'); return; }
  }

  let totalParcelas = null;
  if (tipo === 'parcelado') {
    totalParcelas = parseInt(document.getElementById('rec-total-parcelas').value);
    if (!totalParcelas || totalParcelas < 2) { toast('Informe o total de parcelas (mín. 2)'); return; }
  }
  ...
  toast(_recEditId ? 'Recorrente atualizada ✓' : 'Recorrente criada ✓');
  await carregarPlanejamento();
}

async function excluirRecorrente(id) {
  if (!confirm('Excluir esta recorrente? Lançamentos futuros com situação "previsto" serão removidos.')) return;
  const hoje = new Date().toISOString().split('T')[0];
  // Apagar lançamentos futuros "previsto" vinculados via origem_id
...
async function forcarAtualizarPrevisoes() {
  toast('Atualizando previsões…');
  await gerarLancamentosPrevistos();
  await carregarPlanejamento();
  toast('✓ Previsões atualizadas');
}
...
async function init() {
  ...
  if (!canAccessFinanceiroRole(usr.role)) {
    alert('O módulo financeiro está disponível apenas para sócios e sócios administradores.');
  ...
  document.getElementById('nav-nome').textContent = nomePartes.length <= 2 ? (nomePartes.join(' ')||'—') : nomePartes[0]+' '+nomePartes[nomePartes.length-1];
  const roleMap = { socio:'Sócio', socio_admin:'Sócio', colaborador:'Colaborador', estagiario:'Estagiário', externo:'Externo' };
  const navRole = document.getElementById('nav-role');
  if (navRole) navRole.textContent = roleMap[(usr.role||'').toLowerCase()] || usr.role || '';
  initNavDropdown(usr.role);

  // Sócios: mostrar aba Config, Análise e elementos socio-only
  if (G.isSocio) {
  ...
  // Init Análise ano selector
  initAnaliseAno();
  setAnaliseSubtema(G_analise.subtema || 'resumo');
  ...
  // Mostrar app + inicializar nav lateral compartilhada
  document.getElementById('loading').style.display = 'none';
  document.getElementById('app').style.display = '';

  if (window.ExpNav) {
    ExpNav.init({ module: 'financeiro' });
  }

  // Carregar dashboard inicial
  carregarDashboard();

  // Gerar previsões recorrentes 1x por sessão (silencioso)
