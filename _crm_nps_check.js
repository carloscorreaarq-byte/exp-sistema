...
let allProposals=[];
let allPesquisasSat=[];
let allFinAtivosResumo=[];
let pesquisaSatDisponivel=true;
let finAtivosDisponivel=true;
...
async function carregarDados(){
  const [opsRes, prodsRes, clientesRes, usersRes, contatosRes, fusRes, proposalsRes, pesquisasRes, finAtivosRes] = await Promise.all([
    ...
    sb.from('crm_pesquisa_satisfacao').select('*').then(r=>r.error ? { data: [], error: r.error } : r),
    sb.from('fin_ativos').select('id,oportunidade_id,situacao_financeira').then(r=>r.error ? { data: [], error: r.error } : r),
  ]);
  ...
  if(pesquisasRes.error){
    pesquisaSatDisponivel=false;
    console.warn('carregarDados [crm_pesquisa_satisfacao]:', pesquisasRes.error.message);
  } else {
    pesquisaSatDisponivel=true;
  }
  if(finAtivosRes.error){
    finAtivosDisponivel=false;
    console.warn('carregarDados [fin_ativos]:', finAtivosRes.error.message);
  } else {
    finAtivosDisponivel=true;
  }
  ...
  const pesquisaMap={};
  allPesquisasSat.forEach(r=>{ pesquisaMap[String(r.oportunidade_id||'')]=r; });
  const finOppMap={};
  allFinAtivosResumo.forEach(a=>{
    const k=String(a.oportunidade_id||'');
    if(!k)return;
    if(!finOppMap[k]) finOppMap[k]=[];
    finOppMap[k].push(a);
  });
  ...
  allOps=(opsRes.data||[]).map(op=>({
    ...
    _pesquisaSat: pesquisaMap[String(op.id)] || null,
    _finConcluido: (finOppMap[String(op.id)]||[]).some(a=>isProjetoConcluidoStatus(a.situacao_financeira)),
  }));
}
...
function pesquisaSatBadge(status){
  ...
  if(s==='respondida') return '<span class="badge b-vd">Respondida</span>';
  if(s==='dispensada') return '<span class="badge b-gr">Dispensada</span>';
  return '<span class="badge b-am">Pendente</span>';
}
function pesquisaSatStatusLabel(status){
  const s=normTextKey(status)||'pendente';
  if(s==='respondida') return 'Respondida';
  if(s==='dispensada') return 'Dispensada';
  return 'Pendente';
}
function getPesquisaSat(opId){
  return allPesquisasSat.find(r=>String(r.oportunidade_id||'')===String(opId||''))||null;
}
function isPesquisaSatElegivel(op){
  return !!(op && (isFechada(op) || op._finConcluido));
}
function getPesquisaSatStats(){
  const elegiveis = allOps.filter(isPesquisaSatElegivel);
  const registros = elegiveis.map(op=>getPesquisaSat(op.id)).filter(Boolean);
  const respondidas = registros.filter(r=>normTextKey(r.status)==='respondida');
  const notas = respondidas.map(r=>Number(r.nota)).filter(n=>Number.isFinite(n)&&n>0);
  return {
    elegiveis: elegiveis.length,
    respondidas: respondidas.length,
    pendentes: elegiveis.filter(op=>{
      const r=getPesquisaSat(op.id);
      return !r || normTextKey(r.status)==='pendente';
    }).length,
    notaMedia: notas.length ? (notas.reduce((s,n)=>s+n,0)/notas.length) : null,
    ultimas: respondidas
      .slice()
      .sort((a,b)=>(b.respondido_em||b.updated_at||'').localeCompare(a.respondido_em||a.updated_at||''))
      .slice(0,5)
      .map(r=>{
        const op=allOps.find(o=>String(o.id)===String(r.oportunidade_id));
        return { ...r, _op: op||null };
      })
  };
}
...
function renderDashPesquisaSatisfacao(){
  const el=document.getElementById('nps-body');
  if(!el)return;
  if(!pesquisaSatDisponivel){
    el.innerHTML=`<div class="empty-note">Pesquisa de satisfacao ainda indisponivel. Rode o SQL do DEV-19 para habilitar a base.</div>`;
    return;
  }
  ...
  const ultimas = st.ultimas.length ? `
    <div style="display:flex;flex-direction:column;gap:6px">
      ...
      `).join('')}
    </div>` : `<div class="empty-note">Nenhuma resposta registrada ainda.</div>`;
  el.innerHTML=`
    <div class="g4" style="margin-bottom:10px">
      <div class="stat cinza" style="margin-bottom:0"><div class="stat-lbl">Elegiveis</div><div class="stat-val" style="font-size:18px">${st.elegiveis}</div><div class="stat-sub">oportunidades / projetos finalizados</div></div>
      <div class="stat verde" style="margin-bottom:0"><div class="stat-lbl">Respondidas</div><div class="stat-val" style="font-size:18px">${st.respondidas}</div><div class="stat-sub">pesquisas com retorno</div></div>
      <div class="stat am" style="margin-bottom:0"><div class="stat-lbl">Pendentes</div><div class="stat-val" style="font-size:18px">${st.pendentes}</div><div class="stat-sub">ainda sem resposta fechada</div></div>
      <div class="stat az" style="margin-bottom:0"><div class="stat-lbl">Nota media</div><div class="stat-val" style="font-size:18px">${notaMedia}</div><div class="stat-sub">base aberta, sem retroalimentacao</div></div>
    </div>
    <div style="font-size:8px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:#999;margin-bottom:8px">Ultimas respostas</div>
    ${ultimas}
  `;
}
...
function goTab(id, btn){
  ...
  if(id==='nps')renderDashPesquisaSatisfacao();
    ...
    html += `<div style="border-top:1px solid var(--cinza2);margin-top:8px;padding-top:10px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
      ...
      <div style="display:flex;align-items:center;gap:6px;margin-left:auto">
        ...
        <input type="number" id="fch-vf-total" value="${escAttr(totalProp)}" min="0" step="100"
          style="font-size:12px;font-family:var(--font-mono);border:1px solid var(--azul);padding:4px 8px;width:140px;border-radius:var(--radius-badge,4px);font-weight:700">
      </div>
    </div>`;
  }
...
async function salvarPesquisaSatisfacao(oppId){
  ...
  const mailEl=document.getElementById(`sat-email-${oppId}`);
  const statusEl=document.getElementById(`sat-status-${oppId}`);
  const notaRaw=notaEl?.value||'';
  const payload={
    oportunidade_id: op.id,
    cliente_id: op.cliente_id||null,
    cliente_nome: op._cliente?.nome||null,
    status: statusEl?.value || 'pendente',
    nota: notaRaw ? Number(notaRaw) : null,
    resposta_aberta: respEl?.value.trim() || null,
    contato_referencia: contEl?.value.trim() || null,
    email_referencia: mailEl?.value.trim() || null,
    respondido_em: (statusEl?.value==='respondida') ? new Date().toISOString() : null,
    registrado_por: currentUser?.id || null,
    registrado_por_nome: currentUser?.nome || null,
  };
  if(payload.status==='respondida' && !payload.nota && !payload.resposta_aberta){
    toast('Informe nota ou resposta aberta para marcar como respondida.');
    return;
  }
  const existente=getPesquisaSat(oppId);
  let error=null;
  if(existente){
    ({ error } = await sb.from('crm_pesquisa_satisfacao').update(payload).eq('id',existente.id));
  } else {
    ({ error } = await sb.from('crm_pesquisa_satisfacao').insert(payload));
  }
  if(error){toast('Erro ao salvar pesquisa: '+error.message);return;}
  toast('Pesquisa de satisfacao atualizada.');
  await carregarDados();
  renderDash();
  abrirDetail(oppId);
}
...
async function marcarPesquisaSatisfacaoStatus(oppId,status){
  ...
  if(error){toast('Erro ao atualizar pesquisa: '+error.message);return;}
  toast(status==='dispensada' ? 'Pesquisa dispensada.' : 'Pesquisa marcada como pendente.');
  await carregarDados();
  renderDash();
  abrirDetail(oppId);
}
...
function abrirDetail(oppId){
  ...
  const pesquisa = getPesquisaSat(oppId);
  const elegivelPesquisa = isPesquisaSatElegivel(op);
  const satStatus = pesquisa?.status || 'pendente';
  const satNota = pesquisa?.nota ? String(pesquisa.nota) : '';
  const satResposta = pesquisa?.resposta_aberta || '';
  const satContato = pesquisa?.contato_referencia || op.resp_cliente_nome || '';
  const satEmail = pesquisa?.email_referencia || op.resp_cliente_email || '';
  html+=`<div class="ms"><div class="ms-title">Pesquisa de satisfacao</div>`;
  if(!pesquisaSatDisponivel){
    html+=`<div style="font-size:11px;color:#888;padding:4px 0">Rode o SQL do DEV-19 para habilitar esta frente no CRM.</div>`;
  } else if(!elegivelPesquisa){
    html+=`<div style="font-size:11px;color:#888;padding:4px 0">Disponivel quando a oportunidade estiver fechada ou o projeto estiver concluido no financeiro.</div>`;
  } else {
    html+=`
      <div class="dr"><div class="dk">Status</div><div class="dv" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">${pesquisaSatBadge(satStatus)}${pesquisa?.respondido_em?`<span style="font-size:10px;color:#888">respondida em ${fmtD(pesquisa.respondido_em)}</span>`:''}</div></div>
      <div class="fg fg2 mb8" style="margin-top:10px">
        <div class="fgroup">
          <label>Status da pesquisa</label>
          <select id="sat-status-${oppId}" ${canEdit?'':'disabled'}>
            <option value="pendente"${satStatus==='pendente'?' selected':''}>Pendente</option>
            <option value="respondida"${satStatus==='respondida'?' selected':''}>Respondida</option>
            <option value="dispensada"${satStatus==='dispensada'?' selected':''}>Dispensada</option>
          </select>
        </div>
        <div class="fgroup">
          <label>Nota</label>
          <select id="sat-nota-${oppId}" ${canEdit?'':'disabled'}>
            <option value="">Sem nota</option>
            <option value="1"${satNota==='1'?' selected':''}>1</option>
            <option value="2"${satNota==='2'?' selected':''}>2</option>
            <option value="3"${satNota==='3'?' selected':''}>3</option>
            <option value="4"${satNota==='4'?' selected':''}>4</option>
            <option value="5"${satNota==='5'?' selected':''}>5</option>
          </select>
        </div>
      </div>
      <div class="fg fg2 mb8">
        <div class="fgroup">
          <label>Contato de referencia</label>
          <input type="text" id="sat-contato-${oppId}" value="${escAttr(satContato)}" ${canEdit?'':'disabled'}>
        </div>
        <div class="fgroup">
          <label>E-mail de referencia</label>
          <input type="email" id="sat-email-${oppId}" value="${escAttr(satEmail)}" ${canEdit?'':'disabled'}>
        </div>
      </div>
      <div class="fgroup">
        <label>Resposta aberta</label>
        <textarea id="sat-resposta-${oppId}" ${canEdit?'':'disabled'}>${escHtml(satResposta)}</textarea>
      </div>
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:10px">
        ${canEdit?`
          <button class="btn sm verde" onclick="salvarPesquisaSatisfacao('${escJsStr(oppId)}')">Salvar pesquisa</button>
          <button class="btn sm" onclick="marcarPesquisaSatisfacaoStatus('${escJsStr(oppId)}','pendente')">Marcar pendente</button>
          <button class="btn sm ghost" onclick="marcarPesquisaSatisfacaoStatus('${escJsStr(oppId)}','dispensada')">Dispensar</button>
        `:''}
      </div>
      <div style="font-size:10px;color:#888;margin-top:8px">Leitura aberta, sem retroalimentacao automatica de outros dados.</div>
    `;
  }
  html+=`</div>`;
...
function renderAlcance(){
  ...
  document.getElementById('alc-stats').innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:2px">
      <div style="text-align:center;padding:8px 4px;background:var(--branco);border:1px solid var(--cinza);border-radius:var(--radius-input,6px)">
...
async function abrirLembretePopup() {
  ...
  const _fs = 'width:100%;border:1px solid var(--cinza,#D0CFC9);border-radius:var(--radius-input,6px);padding:7px 10px;font-size:12px;font-family:var(--font-ui);outline:none;box-sizing:border-box';
  const _bs = 'padding:6px 16px;border:1px solid var(--cinza,#D0CFC9);border-radius:var(--radius-input,6px);font-size:12px;cursor:pointer;background:var(--branco,#fff);font-family:var(--font-ui)';
  ...
  pop.innerHTML = `
    <div style="background:var(--branco,#fff);border-radius:var(--radius-card,8px);padding:24px 28px;max-width:380px;width:95%;box-shadow:0 8px 32px rgba(0,0,0,.18)">
