...
async function carregarResultado(prodId) {
  ...
  const etapasDados = etapas.map(e => {
    ...
    return { etapa: e, pct, previsto, realizado, resultado, custoH, horasH, custoL, compResumo, compValorMes, compValorAcumulado, riscoCompetencia };
  });
  ...
  function calcPrazo(etapa) {
    const lib  = etapa.data_liberacao;
    const prev = etapa.data_estimada;
    const conc = etapa.data_conclusao;
    if (!lib) return null;
    const diffDias = (a, b) => Math.round((new Date(b) - new Date(a)) / 86400000);
    const diasPrev = prev ? diffDias(lib, prev) : null;
    const diasReal = conc ? diffDias(lib, conc) : null;
    const atraso   = diasPrev != null && diasReal != null ? diasReal - diasPrev : null;
    return { lib, prev, conc, diasPrev, diasReal, atraso };
  }

  // Cards por etapa (painel direito rosa/verde/vermelho)
  const cardsEtapas = etapasDados.map(({ etapa, pct, previsto, realizado, resultado, custoH, horasH, custoL, compResumo, compValorMes, compValorAcumulado, riscoCompetencia }) => {
    ...
    return `
      <div class="res-card-etapa ${cardCls}">
        ...
        ${(() => {
          const p = calcPrazo(etapa);
          if (!p) return '';
          const fmtD = iso => iso ? new Date(iso + 'T12:00:00').toLocaleDateString('pt-BR', { day:'2-digit', month:'2-digit' }) : '—';
          const atrasoCor  = p.atraso == null ? '#aaa' : p.atraso > 0 ? 'var(--terracota)' : 'var(--verde)';
          const atrasoIcon = p.atraso == null ? '' : p.atraso > 0 ? '⚠ ' : '✓ ';
          const atrasoTxt  = p.atraso == null ? '—'
            : p.atraso === 0 ? 'No prazo'
            : p.atraso > 0  ? `+${p.atraso}d de atraso`
            : `${Math.abs(p.atraso)}d adiantado`;
          return `
...
function _projHistSyncChrome() {
  ...
  ['atas', 'conversa', 'tudo', 'buscar'].forEach(key => {
    ...
    document.getElementById(`projhist-pane-${key}`)?.classList.toggle('active', key === mode);
  });
  ...
  if (btnNovaAta) btnNovaAta.style.display = mode === 'atas' ? '' : 'none';
  ...
  if (mode !== 'atas' && document.getElementById('reg-form-wrap')?.style.display !== 'none') cancelarFormRegistro();
}
...
async function _projHistLoadMode(options = {}) {
  ...
  const mode = _projHistCurrentMode();
  if (mode === 'atas') {
    await carregarRegistrosProduto(prodId);
    return;
  }
  if (mode === 'conversa') {
...
async function carregarRegistrosProduto(prodId) {
  if(!prodId) return;
  ...
  if(!data?.length) { wrap.innerHTML = '<div class="empty-note">Nenhum registro ainda. Crie a primeira entrada usando o botão acima.</div>'; return; }

  G._registros = data.map(r => ({
    ...r,
...
async function salvarEntradaReg() {
  ...
  try {
    ...
    if(!entradaId) {
      const { data, error } = await sb.from('reg_entrada').insert({
      ...
      const { error } = await sb.from('reg_entrada').update({
        etapa_id:      etapaId || null,
        titulo:        titulo  || '',
        participantes: participantes || '',
        corpo:         corpo   || '',
    ...
    await carregarRegistrosProduto(G.produtoAtivo);
